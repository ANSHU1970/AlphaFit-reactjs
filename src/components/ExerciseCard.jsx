...
export default function ExerciseCard(props) {
    ...
    sm:flex-wrap'>
        ...
        sm:items-center sm:flex-wrap gap-x-4'>
            ...
            sm:text-4xl md:text-4xl font-semibold text-slate-400'>
                    0{i+1}
                </h4>
                <h2 className='capitalize whitespace-nowrap
             truncate max-w-full text-lg sm:text-xl md:text-2xl
              flex-1 sm:text-center'>{exercise.name.replaceAll("_"," ")}
                </h2>
            ...
            <div className='grid grid-cols-2 sm:grid-cols-4 
             sm:place-items-center gap-2'>
                    {['reps','rest','tempo'].map(info=>{
                        return (
                            <div key={info} className='flex flex-col p-2 rounded border-[1.5px] border-solid border-slate-900 w-full'>
                                <h3 className='capitalize text-slte-400 text-sm'>{info==='reps'?`${exercise.unit}`:info}</h3>
                    ...
                    boreder-solid border-orange-900 hover:border-orange-600
                     w-full duration-200 '>
